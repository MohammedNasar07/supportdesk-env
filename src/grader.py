...
def grade_classify(ticket: Ticket, action: AgentAction) -> dict:
    ...
    c = 0.95 if action.category.lower() == ticket.expected_category.lower() else 0.05
    p = 0.95 if action.priority.lower() == ticket.expected_priority.lower() else 0.05
...
def grade_triage(ticket: Ticket, action: AgentAction) -> dict:
    ...
    c = 0.95 if action.category.lower() == ticket.expected_category.lower() else 0.05
    p = 0.95 if action.priority.lower() == ticket.expected_priority.lower() else 0.05
    cl = 0.95 if action.needs_clarification == ticket.ambiguous else 0.05
    es = 0.95 if action.escalation == ticket.requires_escalation else 0.05
...
def grade_resolve(ticket: Ticket, action: AgentAction) -> dict:
    ...
    resp = 0.95 if len(action.response) > 50 else 0.05
